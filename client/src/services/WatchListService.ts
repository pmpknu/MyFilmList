...
export default class WatchListService {
   ...
   */
  static async getWatchListsForMovie(
    movieId: number,
    page: number = 0,
    size: number = 20,
    sort: string[]
    ...
    return api.get<Paged<WatchListDto>>(
      `/movies/${movieId}/watchlists${createCrudUri(page, size, sort)}`
    );
  }
   ...
   * @returns {Promise<AxiosResponse<Paged<WatchListDto>>>} List of watchlists
   */
  static async getWatchListsForUser(
    userId: number,
    page: number = 0,
    size: number = 20,
    sort: string[] = []
  ...
  static async createWatchList(
    watchListData: WatchListCreateDto
  ...
  static async updateWatchList(
    id: number,
    watchListData: WatchListUpdateDto
  ): Promise<AxiosResponse<WatchListDto>> {
   ...
   * @returns {Promise<AxiosResponse<WatchListDto>>} Updated watchlist
   */
  static async uploadWatchListPhoto(
    id: number,
    formData: FormData
  ): Promise<AxiosResponse<WatchListDto>> {
