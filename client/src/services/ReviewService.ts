...
export default class ReviewService {
   ...
   */
  static async getReviewsForMovie(
    movieId: number,
    page: number = 0,
    size: number = 20,
    sort: string[] = []
  ): Promise<AxiosResponse<Paged<ReviewWithoutMovieDto>>> {
    return api.get<Paged<ReviewWithoutMovieDto>>(
      `/movies/${movieId}/reviews${createCrudUri(page, size, sort)}`
    );
  }
  ...
  static async searchReviews(
    ...
    return api.post<Paged<ReviewDto>>(
      `/reviews/search${createCrudUri(page, size, sort)}`,
      searchParams
    );
  }
  ...
  static async getReviewsByUser(
    ...
    size: number = 20,
    sort: string[] = []
  ): Promise<AxiosResponse<Paged<ReviewWithoutUserDto>>> {
    return api.get<Paged<ReviewWithoutUserDto>>(
      `/users/${userId}/reviews${createCrudUri(page, size, sort)}`
    );
  }
}
